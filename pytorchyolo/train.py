...
from pytorchyolo.utils.datasets import ListDataset, my_dataset_LMDB
...
def _create_data_loader(
    img_path, batch_size, img_size, n_cpu, multiscale_training=False
):
...
def run():
    ...
    parser.add_argument(
        "-e", "--epochs", type=int, default=300, help="Number of epochs"
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Path to checkpoint file (.weights or .pth). Starts training from"
            " checkpoint model"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--multiscale_training",
        action="store_true",
        help="Allow multi-scale training",
    )
    parser.add_argument(
        "--iou_thres",
        type=float,
    ...
    mini_batch_size = (
        model.hyperparams["batch"] // model.hyperparams["subdivisions"]
    )
