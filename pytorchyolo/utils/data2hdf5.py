...
def search_files(parent_folder, suffix):
    paths = list(
        filter(lambda x: x.suffix == suffix, Path(parent_folder).rglob("**/*"))
    )
    num_files = len(paths)
...
if __name__ == "__main__":
    ...
    # opened_pickle = open(
    #     "data/nuscenes/nuscenes_infos_temporal_train.pkl", "rb"
    # )
    ...
    with h5py.File(save_path, "w") as hf:
        for img_path in sample_paths[
            :30
        ]:  # save only the first 30 images for testing
